...
//sys	CreateProcess(appName *uint16, commandLine *uint16, procSecurity *SecurityAttributes, threadSecurity *SecurityAttributes, inheritHandles bool, creationFlags uint32, env *uint16, currentDir *uint16, startupInfo *StartupInfoEx, outProcInfo *ProcessInformation) (err error) = CreateProcessW
...
//sys	OpenProcessToken(processHandle Handle, desiredAccess uint32, tokenHandle *Token) (err error)
...
//sys	CryptDecodeObject(encodingType uint32, structType *byte, encodedBytes *byte, flenEncodedBytes uint32, flags uint32, decoded unsafe.Pointer, decodedLen *uint32) (err error) = crypt32.CryptDecodeObject
...
//sys	OpenThreadToken(h syscall.Handle, access uint32, openasself bool, token *windows.Token) (err error) = advapi32.OpenThreadToken
...
//sys   QueueUserAPC(address uintptr, thread Handle, dwData uintptr) (err error)
